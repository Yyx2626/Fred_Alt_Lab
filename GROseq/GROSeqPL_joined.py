...
def join_reads(args):
    if "_" in args.file_1:
        idx = args.file_1.index("_")
        outputname = args.file_1[:idx]+".joined.fastq"
    else:
        idx = args.file_1.index(".")
        outputname = args.file_1[:idx]+".joined.fastq"
    os.system("fastq-join {0} {1} -o {2}".format(args.file_1, args.file_2, outputname))
    os.system("mv {0}join {0}".format(outputname))
    os.system("gzip {0}".format(outputname))
    args.file = outputname +".gz"
    return
...
def homer(args):
    if args.custom_genome:
        ...
        os.system("makeTagDirectory conv_rpkm/{0} alignment/{0}.bam -genome {1}.fa".format(args.output,args.genome))
