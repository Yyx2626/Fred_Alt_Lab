...
merge_bw_plot = function(bw_filenames, chr, start, end, num_windows, y_max=NA, mean_col="gray", se_col="black", scatter_col=hsv(0,0,0.2,0.4), scatter_pch=19, scatter_cex=1, scatter_lwd=1, xaxis=0, yaxis=0, should_annotate_V=FALSE, V_domain_range_DF, V_domain_colors=hsv((0:3)/4), name="", output_tsv_filename=NULL){
#	bw_input_list = list()
	...
	if(xaxis==2){
		...
		echo(x_bar_width)
		segments(x_lim[2]-x_bar_width, adjust_y_lim[1] - diff(adjust_y_lim)*0.1, x_lim[2], adjust_y_lim[1] - diff(adjust_y_lim)*0.02, lwd=6, lend="square", xpd=NA)
	}
	if(yaxis==2){
...
#				points(-end(tiles.gr) + (end(tiles.gr)-start(tiles.gr))/(NF+1)*repidx, mcols(tiles.gr)[["rep" %.% repidx]], col=scatter_col, pch=scatter_pch, cex=scatter_cex, lwd=scatter_lwd)
				points(-end(tiles.gr) + (end(tiles.gr)-start(tiles.gr))/2, mcols(tiles.gr)[["rep" %.% repidx]], col=scatter_col, pch=scatter_pch, cex=scatter_cex, lwd=scatter_lwd)
			}
		}
	}else{
		if(should_annotate_V){
...
#				points(start(tiles.gr) + (end(tiles.gr)-start(tiles.gr))/(NF+1)*repidx, mcols(tiles.gr)[["rep" %.% repidx]], col=scatter_col, pch=scatter_pch, cex=scatter_cex, lwd=scatter_lwd)
				points(start(tiles.gr) + (end(tiles.gr)-start(tiles.gr))/2, mcols(tiles.gr)[["rep" %.% repidx]], col=scatter_col, pch=scatter_pch, cex=scatter_cex, lwd=scatter_lwd)
			}
		}
	}
	tiles.gr
...
#		output_tsv_filename = output_prefix %.% "." %.% target_range$family2[i] %.% "." %.% sub("[:-]", "_", coordinate_range) %.% ".track_" %.% k %.% ".tsv"     #  <-- 2020-05-05 added
		now_GRange = merge_bw_plot(now_bw_filenames, chr, start, end, num_windows, y_max=y_max, mean_col=mean_col, se_col=se_col, xaxis=(k==NG)*axis_type, yaxis=axis_type, should_annotate_V=(k==1), V_domain_range_DF=V_domain_range, V_domain_colors=V_domain_colors, output_tsv_filename=NULL)
