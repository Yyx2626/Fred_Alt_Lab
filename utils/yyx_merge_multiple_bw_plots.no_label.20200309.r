...
merge_bw_plot = function(bw_filenames, chr, start, end, num_windows, y_max=NA, mean_col="gray", se_col="black", xaxis=0, yaxis=0, should_annotate_V=FALSE, name=""){
#	bw_input_list = list()
	...
	if(should_reverse_coord){
		if(should_annotate_V){
			with(V_family_range[-1,], rect(-end, y_max*1.05, -start, y_max*1.85, col=hsv((0:3)/4, 0.2, 1), border=NA, xpd=NA))
...
#			text(mean(adjust_x_lim), y_max*1.04, adj=c(0.5,1), name)
		...
		if(should_annotate_V){
			with(V_family_range[-1,], rect(start, y_max*1.05, end, y_max*1.85, col=hsv((0:3)/4, 0.2, 1), border=NA, xpd=NA))
...
for(i in 1:nrow(target_range)){
	...
	for(k in 1:NG){
		...
		merge_bw_plot(now_bw_filenames, chr, start, end, num_windows, y_max=y_max, mean_col=mean_col, se_col=se_col, xaxis=(k==NG)*axis_type, yaxis=axis_type, should_annotate_V=(k==1))
	}
