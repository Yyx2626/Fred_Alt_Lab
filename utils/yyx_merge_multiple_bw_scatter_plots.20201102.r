...
#### 2020-11-01, ask for the original data requested by Nature
####   Nature also requested to remove errorbar for N=2


#### 2020-05-03, modify the merge bw scatter plots:
####   (1) make the scatter points less obvious
####   (2) move axis far away from plots


#### 2020-04-27, Nature editor asked to add original data points as scatter points, in addition to the merged bar plot
...
merge_bw_plot = function(bw_filenames, chr, start, end, num_windows, y_max=NA, mean_col="gray", se_col="black", scatter_col=hsv(0,0,0.2,0.4), scatter_pch=19, scatter_cex=1, scatter_lwd=scatter_cex, scatter_bg=hsv(0,0,1,0.95), xaxis=0, yaxis=0, should_annotate_V=FALSE, V_domain_range_DF, V_domain_colors=hsv((0:3)/4), name="", output_tsv_filename=NULL){
#	bw_input_list = list()
	...
	if(!is.null(output_tsv_filename)){                                    #  <-- 2020-05-05 added
		cat("Now output to " %.% output_tsv_filename %.% " ...\n")
		write.table(tiles.gr, file=output_tsv_filename, row.names=FALSE, quote=FALSE, sep="\t")
	}
	...
	if(xaxis==2){
		...
		echo(x_bar_width)
		segments(x_lim[2]-x_bar_width, adjust_y_lim[1] - diff(adjust_y_lim)*0.1, x_lim[2], adjust_y_lim[1] - diff(adjust_y_lim)*0.1, lwd=6, lend="square", xpd=NA)
	}
	if(yaxis==2){
...
#			text(mean(adjust_x_lim), y_max*1.04, adj=c(0.5,1), name)
		...
		### 2020-04-29 add below for scatter original data points
		if(NF > 1){
			for(repidx in 1:NF){
#				points(-end(tiles.gr) + (end(tiles.gr)-start(tiles.gr))/(NF+1)*repidx, mcols(tiles.gr)[["rep" %.% repidx]], col=scatter_col, pch=scatter_pch, cex=scatter_cex, lwd=scatter_lwd)
				points(-end(tiles.gr) + (end(tiles.gr)-start(tiles.gr))/2, mcols(tiles.gr)[["rep" %.% repidx]], col=scatter_col, pch=scatter_pch, cex=scatter_cex, lwd=scatter_lwd, bg=scatter_bg)
			}
		}
	}else{
		if(should_annotate_V){
...
#			text(mean(adjust_x_lim), y_max*1.04, adj=c(0.5,1), name)
		...
		if(NF > 1){
			for(repidx in 1:NF){
#				points(start(tiles.gr) + (end(tiles.gr)-start(tiles.gr))/(NF+1)*repidx, mcols(tiles.gr)[["rep" %.% repidx]], col=scatter_col, pch=scatter_pch, cex=scatter_cex, lwd=scatter_lwd)
				points(start(tiles.gr) + (end(tiles.gr)-start(tiles.gr))/2, mcols(tiles.gr)[["rep" %.% repidx]], col=scatter_col, pch=scatter_pch, cex=scatter_cex, lwd=scatter_lwd, bg=scatter_bg)
			}
		}
	}
	tiles.gr
...
for(i in 1:nrow(target_range)){
	...
	coordinate_range = chr %.% ":" %.% start %.% "-" %.% end
	output_pdf_filename = output_prefix %.% "." %.% target_range$family2[i] %.% "." %.% sub("[:-]", "_", coordinate_range) %.% ".pdf"
	output_tsv_filename = output_prefix %.% "." %.% target_range$family2[i] %.% "." %.% sub("[:-]", "_", coordinate_range) %.% ".tsv"     #  <-- 2020-05-05 added
	cat("V family = " %.% target_range$family[i] %.% "\n")
	cat("  coordinate_range = " %.% coordinate_range %.% "\n")
 
	cat("Now output pdf " %.% output_pdf_filename %.% " ...\n");
	cat("\tset cex =\t" %.% cex %.% "\n") 

	NG = length(bw_file_groups)
	pdf(output_pdf_filename, width=pdf_width, height=pdf_height)
	par(mar=c(0,0,0,0), mgp=c(2,0.8,0)+2, cex=cex, mfrow=c(NG, 1), oma=c(axis_type*2,axis_type*2,8,0))

	output_DF = character(0)
	for(k in 1:NG){
...
#		output_tsv_filename = output_prefix %.% "." %.% target_range$family2[i] %.% "." %.% sub("[:-]", "_", coordinate_range) %.% ".track_" %.% k %.% ".tsv"     #  <-- 2020-05-05 added
		now_GRange = merge_bw_plot(now_bw_filenames, chr, start, end, num_windows, y_max=y_max, mean_col=mean_col, se_col=se_col, xaxis=(k==NG)*axis_type, yaxis=axis_type, should_annotate_V=(k==1), V_domain_range_DF=V_domain_range, V_domain_colors=V_domain_colors, output_tsv_filename=NULL, scatter_cex=cex)
		if(is.null(dim(output_DF))){
			output_DF = data.frame(chr=as.character(seqnames(now_GRange)), start=start(now_GRange), end=end(now_GRange), mid=mcols(now_GRange)$mid)
		}
		if(all(abs(output_DF$mid - mcols(now_GRange)$mid) < 1e-8)){
			# good
			tmp_DF = mcols(now_GRange)[, c(names(mcols(now_GRange))[grepl("rep.*$", names(mcols(now_GRange)))], "mean", "sd", "se")]
			names(tmp_DF) = "track_" %.% k %.% "." %.% names(tmp_DF)
			output_DF = cbind(output_DF, tmp_DF)
		}else{
			cat("Warning: mid column does not match !? so I skip output for track_" %.% k %.% "\n")
		}
	}
	write.table(output_DF, file=output_tsv_filename, row.names=FALSE, quote=FALSE, sep="\t")
